...
%
% Notes
% This is a modified copy of the trackuplift script adapted to look at a 
% file of the outflow video data.

% ADD PATH FOR TRACKING FUNCTIONS WRITTEN BY JCC/DLB
addpath([pwd,'/tracking']);

% 0. CONTROL PARAMETERS FOR THIS SCRIPT
filenameMP4 = 'D:\data\2B_uplift\Feb_08_Exp_1_HF_H100D7p5.MP4'; % Name of file to process

thfrac = 0.95;  % Fractional value to set threshold for particle finding
                % Note 'median' might be more robust than 0.25*(max-bg)

scaleX   = 30/116 ; % mm distance per horizontal pixel width
scaleY   = 30/154 ; % mm distance per vertical pixel width.  
...
maxdisp = 5;    % Maximum particle displacement per frame 
                % (distance is in pixel widths - somewhat wrongly assumed identical in X and Y) 

% PARTICLE TRACK ANALYSIS
tInit   = 120;   % Timestamp in MP4 data (seconds) for first frame to analyse
tStepFrames= 1;  % WARNING: USE THIS BELOW TO GET tStep FROM FRAMERATE
nSteps  = 4;   % Number of frames (1 step = 1 frame) to consider 

v = VideoReader(filenameMP4);
numFrames = v.Duration.*v.FrameRate;
tStep = tStepFrames * (1/v.FrameRate);

% MOVEMENT / STATIC region analysis
threshSpeed= 1.0; % mm/sec
threshMove = threshSpeed*tStep*(nSteps-1); % Threshold to identify movement. 
